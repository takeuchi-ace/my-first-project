...
export type ItemCondition =
  /** 接待ポイント（0〜100の内部評価。ゴルフの打数ではない）が min 以上。契約の成否は問わない */
  ...
  /**
   * **ゴルフの打数のほう**。相手がいつもの平均より min 打以上よく回った。
   *
   * 接待ポイントとは別の軸。上限が +10 で、腕のあるプレイヤーは
   * すぐ張り付く（実測: 会話技能0.65で中央9打）ので、単独で緩い値にすると
   * 「狙わなくても4割」を超える。上限そのものを狙わせるか、他と重ねて使う。
   */
  | { kind: 'improvement'; min: number }
  /**
   * 接待ポイント（0〜100の内部評価。ゴルフの打数ではない）が max 以下。契約の成否は問わない。
   *
...
export const giftItems: GiftItem[] = [
  ...
  },
  {
    ...
    line: '…要るなら持っていけ。',
    // 接待ポイントではなく**ゴルフの打数**を条件にする（黒田は「スコアが全てを語る」）。
    // 上限+10に近い8打を要るので、会話でも外さない回だけになる（実測 ふつう13%・上手37%）
    hint: '手に馴染みきった握り。相手をいつもより大きく伸ばして、地雷も踏まずに。',
    slot: 'green',
    condition: {
      kind: 'all',
      of: [{ kind: 'improvement', min: 8 }, { kind: 'noHates' }],
    },
  },
  {
    id: 'item_smith',
  ...
  },
  {
    ...
    // 相談ラウンドの接待ポイントは常に100なので、スコアでは条件にならない（実測100%）。
    // 集めた言葉の数を条件にする
  ...
  },
  {
    ...
    // こちらも打数のほう。上限の+10を出した日だけ（実測 ふつう10%・上手28%）。
    // 「ナイスショット！」を言い続ける相手なので、実際に伸びた日が似合う
    hint: '金糸の刺繍で使うのがためらわれる布。相手がいつもより10打よく回った日に。',
    slot: 'wear',
    condition: { kind: 'improvement', min: 10 },
  },
  {
    id: 'item_daimon_2',
    from: 11,
